...
It also answers the question the service list alone cannot: **which environment variable each credential arrives as.** The `exports` row names them, grouped by service, and the `unset` row names the ones this profile does *not* set — the dangerous half, since `exec` overlays without scrubbing and another identity's ambient value survives there. `$MIEN whoami <profile> --json` carries the same thing as an `env` array of `{var, service, set, note}`, which is the form to parse. Neither prints a value, so both work where a policy blocks a secret dump. Do not conclude a service is unsupported because you cannot find its variable — read this row first; that mistake has cost hours.
